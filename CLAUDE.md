...
- **apps/paperlike-agent/** — Swift agent for the DASUNG Paperlike 253. Its main job is clearing `enableDither` on the DASUNG framebuffers (`IOMobileFramebufferAP`, vendor `0x1263` only — never the built-in panel, where removing dithering causes banding). macOS restores the property on reconnect, wake and mode change, so the agent re-applies it on `CGDisplayRegisterReconfigurationCallback` (plus a short bounded burst) and every 2 s as a fallback; this is why closing PaperLikeClient brings the artifacts back. Runs on `make paperlike`. USB serial control (contrast/speed/refresh) is a separate opt-in via `make paperlike-control` and is not needed for the anti-dithering. Every DASUNG output is handled independently, so two Paperlike monitors work unchanged — they share the same EDID `ProductName`, so tell them apart by `SerialNumber`/`YearOfManufacture`, never by name. It also *reports* (never corrects) the gamma table of each DASUNG output in `paperlike status` — a ceiling below 1.0 crushes the grey levels that are the image on e-ink, and is the most destructive host-side setting for these panels; BetterDisplay's software brightness is the usual cause. See `apps/paperlike-agent/RESEARCH.md`.
